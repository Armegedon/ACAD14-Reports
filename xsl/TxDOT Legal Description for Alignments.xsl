...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lx="http://www.landxml.org/schema/LandXML-1.2" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:landUtils="http://www.autodesk.com/land/civil/vcedit" xmlns:lxml="urn:lx_utils">
	<!--Description:Texas DOT Legal Description for Alignments report.
	
This is a sample report form for generating metes and bounds legal descriptions for alignments, based on Texas DOT conventions.  This report form (stylesheet) is not intended to be a replacement for a trained, experienced professional.  It is intended as a example to make the professional more productive.
		...
		<html>
			...
			sub appendLegalDiv(alignName)
				...
				for each alignNode in dataDoc.selectNodes("//Alignment[@name='" + alignName + "']")
					legalDesc = legalDesc + "Alignment: " + alignNode.getAttributeNode("name").text + "<br/>"
					if(chk = "ON") then
