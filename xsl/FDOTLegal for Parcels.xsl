...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lx="http://www.landxml.org/schema/LandXML-1.2" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:landUtils="http://www.autodesk.com/land/civil/vcedit" xmlns:lxml="urn:lx_utils">
	<!--Description:Florida DOT Legal Description for Parcels report.
	
This is a sample report form for generating metes and bounds legal descriptions for parcels, based on Florida DOT conventions.  This report form (stylesheet) is not intended to be a replacement for a trained, experienced professional.  It is intended as a example to make the professional more productive.
	...
	<xsl:template match="/">
		...
		<html>
			...
			function GetHeader(parcelName)
				...
				headStr = "<table>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>PARCEL NO.</b></td>"
				headStr = headStr + "<td>" + parcelName + "</td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>FIN NO.</b></td>"
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Florida_DOT.FIN_Number"/></td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>SECTION</b></td>"
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Section_Information.Section"/></td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>STATE ROAD</b></td>"
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Florida_DOT.State_Road"/></td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>COUNTY </b></td>"
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Florida_DOT.County"/></td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "</table>"
				
				headStr = headStr + "<p/>"
				headStr = headStr + "A parcel of land in Section " 
				headStr = headStr + "<xsl:value-of select="$Legal_Descriptions.Section_Information.Section"/>"
				headStr = headStr  + " Township "
				headStr = headStr  + "<xsl:value-of select="$Legal_Descriptions.Section_Information.Township"/> "
				headStr = headStr  +  "<xsl:value-of select="$Legal_Descriptions.Section_Information.Baseline_Direction"/>"
				headStr = headStr  + ", Range "
				headStr = headStr  + "<xsl:value-of select="$Legal_Descriptions.Section_Information.Range"/> "
				headStr = headStr  + "<xsl:value-of select="$Legal_Descriptions.Section_Information.Meridian_Direction"/>"
				headStr = headStr  + ", being more particularly described as follows:<p/>"
				...
				for each parcelNode in dataDoc.selectNodes("//Parcel[@name='" + parcelName + "']")
					if(aliUse > 0) then
						...
						for each aliNode in dataDoc.selectNodes("//Alignment[@name='" + aliName + "']")
							legalDesc = legalDesc + aliNode.text + "<p/>"
						next
					end if
					if(chk = "ON") then
					...
					legalDesc = legalDesc + "<p>" + parcelNode.getAttributeNode("area").text + " more or less</p><hr/>"
				next
			...
			sub hideInput
				if bIsShrunk = false then
					...
					Document.all("titleDiv").innerText = "Florida DOT Legal Description (Click to collapse or expand)"
					Document.all("input").style.overflow = "auto"
					bIsShrunk = false
				end if
				outputScrolled
	...
	<xsl:template name="InsertForm">
		<form id="appendForm">
			<b><div id="titleDiv" onclick="hideInput" style="background-color: silver">
				Florida DOT Legal Description <br/>(Click to collapse or expand)
			</div></b>
			<p/>
