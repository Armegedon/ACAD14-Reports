...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lx="http://www.landxml.org/schema/LandXML-1.2" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:landUtils="http://www.autodesk.com/land/civil/vcedit" xmlns:lxml="urn:lx_utils">
	<!--Description:Florida DOT Legal Description for Alignments report.
	
This is a sample report form for generating metes and bounds legal descriptions for alignments, based on Florida DOT conventions.  This report form (stylesheet) is not intended to be a replacement for a trained, experienced professional.  It is intended as a example to make the professional more productive.
...
This form is valid for LandXML 0.88, 1.0, 1.1 and 1.2 data.-->
	...
	<xsl:template match="/">
		...
		<html>
			...
			function GetFooter
				Dim footStr
				footStr = "<p>SURVEYED ON THE GROUND " + "<xsl:value-of select="$Legal_Descriptions.Header_Information.Survey_date"/>" + "</p><br/>"
				footStr = footStr + "<hr/>"
			...
			sub appendLegalDiv(alignName)
				...
				for each alignNode in dataDoc.selectNodes("//Alignment[@name='" + alignName + "']")
					legalDesc = legalDesc + alignNode.getAttributeNode("name").text + "<br/>"
					if(chk = "ON") then
