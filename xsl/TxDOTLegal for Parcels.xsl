...
<!--Description:Texas DOT Legal Description for Parcels report.
	
This is a sample report form for generating metes and bounds legal descriptions for parcels, based on Texas DOT conventions.  This report form (stylesheet) is not intended to be a replacement for a trained, experienced professional.  It is intended as a example to make the professional more productive.
...
This form is valid for LandXML 0.88, 1.0, 1.1 and 1.2 data.-->
	...
	<xsl:template match="/">
		...
		<html>
			<head>
				<title>TxDOT Legal Description Generation</title>
			...
			function GetHeader(parcelName)
				...
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Texas_DOT.County"/></td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>Account No:</b></td>"
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Texas_DOT.Account_Number"/></td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>CSJ No:</b></td>"
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Texas_DOT.CSJ_Number"/></td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>Highway No:</b></td>"
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Texas_DOT.Hwy_Number"/></td>"
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				...
				headStr = headStr + "</tr>"
				headStr = headStr + "<tr>"
				headStr = headStr + "<td><b>Grantor:</b></td>"
				headStr = headStr + "<td><xsl:value-of select="$Legal_Descriptions.Texas_DOT.Grantor"/></td>"
				headStr = headStr + "</tr>"
			...
			function GetFooter
				Dim footStr
				footStr = "<p>SURVEYED ON THE GROUND " + "<xsl:value-of select="$Legal_Descriptions.Header_Information.Survey_date"/>" + "<br/>"
				footStr = footStr + "<xsl:value-of select="$Legal_Descriptions.Header_Information.Surveyor"/></p>"
			...
			sub appendLegalDiv(parcelName)
				...
				for each parcelNode in dataDoc.selectNodes("//Parcel[@name='" + parcelName + "']")
					...
					legalDesc = legalDesc + parcelNode.getAttributeNode("area").text
					legalDesc = legalDesc  + GetFooter
			...
			sub hideInput
				if bIsShrunk = false then
					...
					Document.all("titleDiv").innerText = "Texas DOT Legal Description (Click to collapse or expand)"
					Document.all("input").style.overflow = "auto"
					bIsShrunk = false
				end if
				outputScrolled
	...
	<xsl:template name="InsertForm">
		<form id="appendForm">
			<b><div id="titleDiv" onclick="hideInput" style="background-color: silver">
				Texas DOT Legal Description <br/>(Click to collapse or expand)
			</div></b>
			<p/>
